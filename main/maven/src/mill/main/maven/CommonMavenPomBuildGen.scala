...
trait CommonMavenPomBuildGen[Config <: BuildGenConfig] extends CommonBuildGen[Config] {
  ...
  override def generateMillNodeTree(workspace: Path, cfg: Config): Tree[MillNode] = {
    val input: Tree[Node[Model]] = getMavenNodeTree(workspace, cfg)

    convert(input, cfg)
  }

  def getMavenNodeTree(workspace: Path, cfg: Config): Tree[MavenNode]

  private def convert(input: Tree[MavenNode], cfg: Config): Tree[MillNode] = {
  ...
  private def metadata(model: Model, cfg: Config): String = {
