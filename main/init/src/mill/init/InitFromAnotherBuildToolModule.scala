...
/**
 * Defines a [[InitModule.init task]] to convert a build of another build tool to Mill.
 */
@mill.api.experimental
trait InitFromAnotherBuildToolModule extends TaskModule {
  def defaultCommandName(): String = "init"

  def buildGenClasspathArtifact: String
  ...
  def buildGenClasspath: T[Loose.Agg[PathRef]] = T {
    ...
    Util.millProjectModule(buildGenClasspathArtifact, repositories)
  }
  ...
  /**
   * Generates and formats Mill build files for an existing project of another build tool.
   *
  ...
  def init(args: String*): Command[Unit] = Task.Command {
    ...
    if (exit == 0) {
      ...
      throw InitFromAnotherBuildToolException(s"$mainClass exit($exit)")
    }
  }
}

@mill.api.experimental
case class InitFromAnotherBuildToolException(message: String) extends Exception(message)
    with NoStackTrace
